...
class MimasV2ConfigDownloader:
	...
	def M25P16sectorErase(self, StartAddress, EndAddress):
		'''Erases sectors from the sector that contains StartAddressup
           up to the sector that contains EndAddress'''
		StartAddress -= (StartAddress & 0xFFFF)
		EndAddress |= 0xFFFF;
		
		i = 0
		
		for i in range(StartAddress, EndAddress, 0xFFFF):
	...
	def M25P16VerifyFlash(self, buffer, StartAddress):
		...
		#Send address bytes
		address = struct.pack("i", StartAddress)
	...
	def ConfigureMimasV2(self, FileName, StartAddress = 0):
		'Configures Mimas V2'
		...
		if FlashAlgorithm == FLASH_ALGORITHM_M25P16:
			...
			if self.M25P16sectorErase(StartAddress, fileSize):
			...
			while fileSize:
				...
				self.M25P16PageProgram(dataBuff[address:address+j], StartAddress + address, j)
				address += j
				fileSize -= j
			...
			if self.M25P16VerifyFlash(dataBuff, StartAddress):
...
def main():
	...
	if(argc not in [3,4]):
		...
		print("Usage : mimasv2config.py <PORT> <Binary File> [Start Address]\n")
		...
		print("your design.\n")
		print("Start Address - The base address for the binary.")
		exit(1)
	...
	if(argc == 3):
		...
		MimasV2configObj.ConfigureMimasV2(sys.argv[2], int(sys.argv[3]))
